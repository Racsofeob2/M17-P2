                                            ...
                                            if(isset($_GET['search']) and $_GET['search'] != "" )
                                            {
                                                if (isset($_GET['search']) && $_GET['search'] != "") {
                                                   if (!empty($_GET['search'])) {
                                                       $search = "%" . $_GET['search'] . "%"; // Añade los comodines de búsqueda
                                                       $stmt = $mysqli->prepare("SELECT id, username, email, name, surname FROM users WHERE name LIKE ?");
                                                       $stmt->bind_param("s", $search); // "s" indica que el parámetro es un string
                                                       $stmt->execute();
                                                   ...
                                                   while ($list = $result->fetch_assoc()) {
                                                       ...
                                                       <tr>
                                                           ...
                                                           <td>' . htmlspecialchars($list['email']) . '</td>
                                                           <td>' . htmlspecialchars($list['name']) . '</td>
                                                           <td>' . htmlspecialchars($list['surname']) . '</td>
                                                       </tr>';
                                                   }
                                                }
